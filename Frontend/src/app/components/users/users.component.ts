...
export class UsersComponent implements OnInit {
  ...
  parents = null;   //Store parents data to show when adding new student
  ...
  ngOnInit() {
    ...
    this.api.get('users?&role=Parent', this.headers).subscribe(
      data => this.showGuardians(data),
      error => { console.log(error); this.notify.error(error.error.message); }
    );
  }
  ...
  addModalSubmit(){
    ...
    if(this.role == "Student"){
      this.form = Object.assign(this.form, this.student);
    } else if(this.role = "Teacher"){
      this.form = Object.assign(this.form, this.teacher);
    }
    if(!this.showRole){
      this.form.role.push(this.role);
    }
